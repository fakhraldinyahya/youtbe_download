...
async function getVideoInfo() {
    ...
    try {
        ...
        updateQualityOptions(data.available_formats);
...
async function downloadVideo() {
    const url = youtubeUrlInput.value.trim();
    const qualityOption = qualitySelect.value;
    ...
    try {
        const format_id = qualityOption === 'highest' ? '' : qualityOption;
...
function updateQualityOptions(formats) {
    ...
    if (formats && formats.length > 0) {
        // إضافة عنوان لفصل الخيارات
        const optgroup = document.createElement('optgroup');
        optgroup.label = 'الجودات المتاحة';
